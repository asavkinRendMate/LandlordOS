...
import { checkInCompleteHtml } from '@/lib/email-templates'
...
export async function generateCheckInPdf(reportId: string): Promise<void> {
  ...
  const tenantName = report.tenant?.name ?? 'Tenant'

  // Cover page
  const coverPage = pdf.addPage([PAGE_W, PAGE_H])
  let y = PAGE_H - 100

  coverPage.drawText('Check-in Report', { x: MARGIN, y, font: fontBold, size: 28, color: GREEN })
  y -= 40
  ...
  const details = [
    ...
    ['Tenant', tenantName],
    ['Date', new Date(report.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })],
    ['Landlord confirmed', report.landlordConfirmedAt ? new Date(report.landlordConfirmedAt).toLocaleDateString('en-GB') : 'Pending'],
    ['Tenant confirmed', report.tenantConfirmedAt ? new Date(report.tenantConfirmedAt).toLocaleDateString('en-GB') : 'Pending'],
  ...
  y -= 30
  coverPage.drawText(`Total photos: ${report.photos.length}`, { x: MARGIN, y, font, size: 11, color: rgb(0.4, 0.4, 0.4) })
  y -= 18
  const landlordPhotos = report.photos.filter((p) => p.uploadedBy === 'LANDLORD').length
  const tenantPhotos = report.photos.filter((p) => p.uploadedBy === 'TENANT').length
  coverPage.drawText(`By landlord: ${landlordPhotos}  |  By tenant: ${tenantPhotos}`, { x: MARGIN, y, font, size: 11, color: rgb(0.4, 0.4, 0.4) })

  // Group photos by room
  const roomGroups = new Map<string, { roomName: string; photos: typeof report.photos }>()
  ...
  for (const [, group] of Array.from(roomGroups)) {
    ...
    for (const photo of group.photos) {
      ...
      try {
        const signedUrl = await getCheckInPhotoUrl(photo.fileUrl)
  ...
  const storagePath = `check-in-reports/${reportId}/check-in-report.pdf`
  ...
  if (report.tenant?.email) {
    ...
    const downloadUrl = signedUrlData?.signedUrl ?? `${env.NEXT_PUBLIC_APP_URL}/check-in/${report.token}`

    await sendEmail({
      to: report.tenant.email,
      subject: `Your check-in report is ready — ${address}`,
      html: checkInCompleteHtml({
