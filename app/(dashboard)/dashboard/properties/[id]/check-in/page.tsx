...
export default function CheckInPage() {
  ...
  const fetchReport = useCallback(async () => {
    if (!reportId) return
    try {
      const res = await fetch(`/api/check-in/${reportId}`)
  ...
  async function handleUpload(files: FileList) {
    ...
    for (const rawFile of Array.from(files)) {
      ...
      await fetch(`/api/check-in/${reportId}/photos`, {
        method: 'POST',
        body: formData,
      })
    }
  ...
  async function deletePhoto(photoId: string) {
    if (!reportId) return
    await fetch(`/api/check-in/${reportId}/photos/${photoId}`, { method: 'DELETE' })
    await fetchReport()
  }
  ...
  async function sendToTenant() {
    ...
    if (!report.tenant && tenants.length > 0) {
      await fetch(`/api/check-in/${reportId}`, {
        method: 'PATCH',
  ...
  return (
    <div className="p-4 lg:p-8 max-w-4xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-6">
        <div>
          <h1 className="text-[#1A1A1A] text-2xl font-bold">Check-in Report</h1>
