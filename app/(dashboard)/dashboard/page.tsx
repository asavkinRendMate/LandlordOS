...
function complianceColor(doc: ComplianceDoc): string {
  if (doc.type === 'HOW_TO_RENT') {
    return doc.issued ? 'bg-green-400' : 'bg-white/20'
  }
  switch (doc.status) {
    case 'VALID':
      return 'bg-green-400'
...
function PropertyCard({ property }: { property: PropertyWithRelations }) {
  ...
  const complianceDocs = ['GAS_SAFETY', 'EPC', 'EICR', 'HOW_TO_RENT'].map((type) =>
    property.complianceDocs.find((d) => d.type === type),
  )

  return (
    <Link
      href={`/dashboard/properties/${property.id}`}
      className="block bg-white/4 border border-white/8 rounded-xl p-5 hover:bg-white/6 hover:border-white/14 transition-all group"
    >
      {/* Header row */}
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="min-w-0">
          <p className="text-white font-medium text-sm leading-snug truncate">{property.line1}</p>
          <p className="text-white/40 text-xs mt-0.5">
            {property.city}, {property.postcode}
          </p>
        </div>
        <span className={`shrink-0 text-xs font-medium px-2 py-1 rounded-full ${config.badge}`}>
        ...
        {activeTenancy ? (
          <>
            ...
            {activeTenancy.tenantName && (
              <span className="ml-1.5">· {activeTenancy.tenantName}</span>
            )}
          </>
        ) : (
      ...
      <div className="flex items-center gap-2">
        {complianceDocs.map((doc, i) => {
          const type = ['GAS_SAFETY', 'EPC', 'EICR', 'HOW_TO_RENT'][i]
...
export default async function DashboardPage() {
  ...
  return (
    <div className="p-8">
      {/* Alert bar */}
      {alerts.length > 0 && (
        <div className="mb-6 bg-yellow-500/10 border border-yellow-500/25 rounded-xl px-4 py-3">
          {alerts.map((a, i) => (
            <p key={i} className="text-yellow-300 text-sm">
              <span className="font-medium">{a.type} certificate</span> expires{' '}
              {a.expiryDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })} —{' '}
              {a.property}
            </p>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Link
          ...
          className="flex items-center gap-1.5 bg-green-500 hover:bg-green-400 text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors"
        >
