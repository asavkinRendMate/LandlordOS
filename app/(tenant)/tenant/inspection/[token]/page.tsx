...
export default function TenantInspectionReview() {
  ...
  const fetchReport = useCallback(async () => {
    try {
      const res = await fetch(`/api/inspections/token/${token}`)
  ...
  async function uploadStaged() {
    ...
    try {
      ...
      await fetch(`/api/inspections/token/${token}/photos`, {
        method: 'POST',
        body: formData,
      })
      cancelStaged()
  ...
  async function handleConfirm() {
    setConfirming(true)
    try {
      const res = await fetch(`/api/inspections/token/${token}/confirm`, {
        method: 'POST',
    ...
    <div className="min-h-screen bg-[#F7F8F6]">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-[#1A1A1A] text-2xl font-bold mb-2">Property Inspection</h1>
